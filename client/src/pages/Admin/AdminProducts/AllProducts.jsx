  ...
  useEffect(() => {
    const deleteHandler = async () => {
      if (verifyDelete === true) {
        await axios
          .delete(
            `http://localhost:8000/api/Add/${section}/remove/${deleteProduct}`
          )
          .then((res) => {
            setProduct((prevProduct) =>
              prevProduct.filter((item) => item._id !== deleteProduct)
            );
